...
- **Label visibility.** Select a segmentation label to show only that label, or **All labels** to restore the complete overlay. Filtering is also available on read-only cases and never changes the stored mask. The separate **Brush label** selector chooses the painting target; when isolating a label, changing the brush label also makes that target visible.
- **Brush preview.** Brush marks made during the current viewer session remain visible at 0% overlay (or with segmentation hidden), so you can paint over the original CT. Marks follow the label filter, remain visible across slices, planes, and window changes, and are restored by Undo. Save or Discard clears this preview; saved segmentation follows the normal overlay opacity.
